...
const SCOPES = "https://mail.google.com/ https://www.googleapis.com/auth/gmail.labels https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/gmail.modify https://www.googleapis.com/auth/gmail.readonly"
...
function User() {
  ...
  const getLabelId = (array) => {
    const audioList = array.labels.filter((listIds) => listIds.name === "toAudio")
  ...
  const getEmailsList = (listId) => {
    let accessToken = gapi.auth.getToken().access_token;
    fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/?labelIds=${listId}`, {
      method: "GET",
      headers: new Headers({ 'Authorization': 'Bearer ' + accessToken})
    }).then((res)=> {
      return res.json()
